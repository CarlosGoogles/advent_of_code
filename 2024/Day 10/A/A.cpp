...
unordered_set<string> dfs(const vector<string>& mat, ii p, foo& vis) {
    string s = tostring(p);
    int act = mat[p.first][p.second];
    
    if (act == '9') {
        return unordered_set<string>{tostring(p)};
    }
